...
var bullet_scene : PackedScene = preload("res://scenes/movable/projectiles/fantasy bullet.tscn")

# Shooting parameters
var shoot_speed : float = 4.0
var shoot_interval : float = 0.125  # Minimum time between shots
var bullet_lifetime : float = 4.0
var can_shoot = true
...
func shoot():
	...
	if shoot_plane_point != Vector3():
		...
		var shoot_direction = (shoot_plane_point - global_transform.origin*Vector3(1,1,0)).normalized()
		...
		$"../..".add_child(bullet)
		bullet.add_to_group("fantasy_bullet", true)
